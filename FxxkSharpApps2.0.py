# coding: UTF-8
import tkinter as tk
import webbrowser
import platform
import os
from tkinter import messagebox

if not os.name == 'nt':messagebox.showerror('Attention','Windows以外のOSでの実行は想定されていません。エラーが発生しても自己責任でお願いします。')

ur = platform.uname()
print(ur.system)
print(ur.release)
print(ur.version)
print(ur.processor)

if ur.release == 'xp':messagebox.showerror('Attention','このバージョンのWindowsは対応していません。')
if ur.release == '2000':messagebox.showerror('Attention','このバージョンのWindowsは対応していません。')
if ur.release == 'me':messagebox.showerror('Attention','このバージョンのWindowsは対応していません。')
if ur.release == '98':messagebox.showerror('Attention','このバージョンのWindowsは対応していません。')
if ur.release == '95':messagebox.showerror('Attention','このバージョンのWindowsは対応していません。')

print('FxxkSHARPApps_V2.1')
print('Developer @wakanameko2')

baseGround = tk.Tk()

if ur.release == 'vista':baseGround.geometry('600x75')
if ur.release == '7':baseGround.geometry('600x75')
if ur.release == '8':baseGround.geometry('600x75')
if ur.release == '8.1':baseGround.geometry('600x75')
if ur.release == '2012ServerR2':baseGround.geometry('600x75')
if ur.release == '10':baseGround.geometry('500x50')
#Windows11
if ur.version == '10.0.22000':baseGround.geometry('575x70')
if ur.version == '10.0.22621':baseGround.geometry('575x70')

baseGround.resizable(width = False, height = False)

baseGround.title('FxxkSHARPApps')

data = '''
     '''
baseGround.tk.call('wm', 'iconphoto', baseGround._w, tk.PhotoImage(data=data))
    
def btn_click():    
    label1 = tk.Label(baseGround, text='Selected Uninstall').place(x=290,y=15)
    print('selected Uninstall.')
    OAU = messagebox.askyesno('確認','おサイフケータイ機能を残しますか？')
    if OAU == True:
            print('Selected Yes')
            import subprocess
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.settingsaquosusefull')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.scshocapture2')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'sharp.jp.android.makersiteappli')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.iwnnime.ml')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shframecapture')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.karadamate')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.pseudlock.helper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent2')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.sharpdatabackup')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.scrollauto')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shrobodockservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.SHWirelessOutput')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mb')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mc')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.LockWeatherWidget')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.startuptutorial_ps')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.friendlink')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.service.hikarieffect')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shlivewallpaper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopa.overlayservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopa.systemservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.wallpaper3d')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.lockwallpaperchanger')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'com.mobisystems.office')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.antitok')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.pseudolock.helper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingsuw')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingapp')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.usbchargingalart')
            subprocess.call(cmd)
            print('All_Done')
            label1 = tk.Label(baseGround, text='All Done              ').place(x=290,y=15)]
    else:
            print('Selected No')
            import subprocess
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.settingsaquosusefull')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.scshocapture2')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'sharp.jp.android.makersiteappli')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.iwnnime.ml')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shframecapture')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.karadamate')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.pseudlock.helper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent2')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.sharpdatabackup')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.scrollauto')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shrobodockservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.SHWirelessOutput')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mb')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mc')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.LockWeatherWidget')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.startuptutorial_ps')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.friendlink')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.service.hikarieffect')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.shlivewallpaper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopa.overlayservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.emopa.systemservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.wallpaper3d')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.lockwallpaperchanger')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'com.mobisystems.office')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.antitok')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.nfcsettings')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.pseudolock.helper')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingservice')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingsuw')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.kittingapp')
            subprocess.call(cmd)
            cmd = ('adb', 'shell', 'pm', 'uninstall', '-k', '--user', '0', 'jp.co.sharp.android.usbchargingalart')
            subprocess.call(cmd)
            print('All_Done')
            label1 = tk.Label(baseGround, text='All Done              ').place(x=290,y=15)
    
def btn2_click():
    label1 = tk.Label(baseGround, text='Selected Install   ').place(x=290,y=15)
    print('selected Install.')
    import subprocess
    
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.settingsaquosusefull')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.scshocapture2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'sharp.jp.android.makersiteappli')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.iwnnime.ml')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.shframecapture')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.karadamate')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.pseudlock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.emopar')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.emopar.internalcontent')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.emopar.internalcontent2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.sharpdatabackup')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.scrollauto')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.shrobodockservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.SHWirelessOutput')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.shinstructionmanual02mb')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.shinstructionmanual02mc')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.LockWeatherWidget')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.startuptutorial_ps')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.friendlink')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.service.hikarieffect')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.shlivewallpaper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.emopa.overlayservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.emopa.systemservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.wallpaper3d')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.lockwallpaperchanger')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'com.mobisystems.office')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.antitok')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.nfcsettings')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.pseudolock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.kittingservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.kittingsuw')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.kittingapp')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'cmd', 'package', 'install-existing', 'jp.co.sharp.android.usbchargingalart')
    subprocess.call(cmd)
    print('All_Done')
    label1 = tk.Label(baseGround, text='All Done              ').place(x=290,y=15)

def btn3_click():
    label1 = tk.Label(baseGround, text='Selected Disable  ').place(x=290,y=15)
    print('selected Disable.')
    import subprocess
    
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.settingsaquosusefull')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.scshocapture2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'sharp.jp.android.makersiteappli')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.iwnnime.ml')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.shframecapture')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.karadamate')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.pseudlock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.emopar')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.sharpdatabackup')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.scrollauto')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.shrobodockservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.SHWirelessOutput')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mb')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mc')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.LockWeatherWidget')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.startuptutorial_ps')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.friendlink')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.service.hikarieffect')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.shlivewallpaper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.emopa.overlayservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.emopa.systemservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.wallpaper3d')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.lockwallpaperchanger')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'com.mobisystems.office')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.antitok')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.nfcsettings')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.pseudolock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.kittingservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.kittingsuw')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.kittingapp')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'disable-user', '--user', '0', 'jp.co.sharp.android.usbchargingalart')
    subprocess.call(cmd)
    print('All_Done')
    label1 = tk.Label(baseGround, text='All Done              ').place(x=290,y=15)

def btn4_click():
    label1 = tk.Label(baseGround, text='Selected Enable   ').place(x=290,y=15)
    print('selected Enable.')
    import subprocess
           
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.settingsaquosusefull')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.scshocapture2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'sharp.jp.android.makersiteappli')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.iwnnime.ml')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.shframecapture')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.karadamate')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.pseudlock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.emopar')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.emopar.internalcontent2')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.sharpdatabackup')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.scrollauto')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.shrobodockservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.SHWirelessOutput')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mb')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.shinstructionmanual02mc')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.LockWeatherWidget')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.startuptutorial_ps')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.friendlink')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.service.hikarieffect')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.shlivewallpaper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.emopa.overlayservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.emopa.systemservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.wallpaper3d')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.lockwallpaperchanger') 
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'com.mobisystems.office')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.antitok')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.nfcsettings')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.pseudolock.helper')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.kittingservice')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.kittingsuw')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.kittingapp')
    subprocess.call(cmd)
    cmd = ('adb', 'shell', 'pm', 'enable', '--user', '0', 'jp.co.sharp.android.usbchargingalart')
    subprocess.call(cmd)
    print('All_Done')
    label1 = tk.Label(baseGround, text='All Done              ').place(x=290,y=15)
    
def btn_click_adb():
    webbrowser.open('https://dl.google.com/android/repository/platform-tools_r33.0.1-darwin.zip')

if ur.release == 'vista':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=253,y=48)
if ur.release == '7':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=253,y=48)
if ur.release == '2008ServerR2':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=238,y=41)
if ur.release == '8':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=290,y=48)
if ur.release == '8.1':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=290,y=48)
if ur.release == '2012ServerR2':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=290,y=48)
#Windows10
if ur.version == '10.0.10240':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.10586':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.14393':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.15063':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.16299':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.17134':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.17763':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.18362':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.18363':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.19041':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.19042':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.19043':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.19044':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
if ur.version == '10.0.19045':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=225,y=36)
#Windows11
if ur.version == '10.0.22000':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=270,y=43)
if ur.version == '10.0.22621':label1 = tk.Label(baseGround, text='このソフトフェアを使用するにはADBが必要です。').place(x=270,y=43)

if ur.release == 'vista':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 530, y=41)
if ur.release == '7':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 530, y=41)
if ur.release == '2008ServerR2':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 464, y=38)
if ur.release == '8':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 525, y=43)
if ur.release == '8.1':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 525, y=43)
if ur.release == '2012ServerR2':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 525, y=43)
#Windows10
if ur.version == '10.0.10240':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.10586':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.14393':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.15063':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.16299':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.17134':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.17763':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.18362':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.18363':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.19041':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.19042':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.19043':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.19044':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
if ur.version == '10.0.19045':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 450, y=30)
#Windows11
if ur.version == '10.0.22000':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 500, y=40)
if ur.version == '10.0.22621':button_adb = tk.Button(
    baseGround, text='Download', command=btn_click_adb).place(x= 500, y=40)

button = tk.Button(
    baseGround, text='Uninstall', command=btn_click).place(x= 10, y=10)

button2 = tk.Button(
    baseGround, text='Install', command=btn2_click).place(x= 90, y=10)
 
button3 = tk.Button(
    baseGround, text='Disable', command=btn3_click).place(x= 155, y=10)

button4 = tk.Button(
    baseGround, text='Enable', command=btn4_click).place(x= 225, y=10)
    
baseGround.mainloop()
